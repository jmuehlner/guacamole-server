...
} guac_socket_handle_data;

/**
 * Writes the entire contents of the given buffer to the file handle
 * associated with the given socket, retrying as necessary until the whole
 * buffer is written, and aborting if an error occurs.
 *
 * @param socket
 *     The guac_socket associated with the file handle to which the given
 ...
 */
static ssize_t guac_socket_handle_write(guac_socket* socket,
    ...
    while (count > 0) {
        ...
        */
        ...
        if (!WriteFile(data->handle, buf, count, NULL, &overlapped)) {
...
/**
 * Attempts to read from the underlying file handle of the given
 ...
 */
static ssize_t guac_socket_handle_read_handler(guac_socket* socket,
 ...
 */
static ssize_t guac_socket_handle_flush(guac_socket* socket) {
    ...
    if (data->written > 0) {
        ...
        if (guac_socket_handle_write(socket, data->out_buf, data->written))
            return 1;

        /* Flush the handle buffers */
 ...
 *     Zero if the flush operation was successful, non-zero otherwise.
 */
static ssize_t guac_socket_handle_flush_handler(guac_socket* socket) {
    ...
    retval = guac_socket_handle_flush(socket);
 ...
 *     during write.
 */
static ssize_t guac_socket_handle_write_buffered(guac_socket* socket,
        const void* buf, size_t count) {
    ...
    while (count > 0) {
        ...
        if (remaining == 0) {
            ...
            if (guac_socket_handle_flush(socket))
                return -1;
 ...
 */
static ssize_t guac_socket_handle_write_handler(guac_socket* socket,
        const void* buf, size_t count) {
    ...
    retval = guac_socket_handle_write_buffered(socket, buf, count);
...
/**
 * Waits for data on the underlying file handle of the given socket to
 ...
 */
static int guac_socket_handle_select_handler(guac_socket* socket,
        int usec_timeout) {
 ...
 */
static int guac_socket_handle_free_handler(guac_socket* socket) {
 ...
 */
static void guac_socket_handle_lock_handler(guac_socket* socket) {
 ...
 */
static void guac_socket_handle_unlock_handler(guac_socket* socket) {
...
guac_socket* guac_socket_open_handle(HANDLE handle) {
    ...
    socket->read_handler   = guac_socket_handle_read_handler;
    socket->write_handler  = guac_socket_handle_write_handler;
    socket->select_handler = guac_socket_handle_select_handler;
    socket->lock_handler   = guac_socket_handle_lock_handler;
    socket->unlock_handler = guac_socket_handle_unlock_handler;
    socket->flush_handler  = guac_socket_handle_flush_handler;
    socket->free_handler   = guac_socket_handle_free_handler;
